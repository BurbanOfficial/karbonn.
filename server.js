...
// ===========================
// Cron: expiration reminders
// ===========================

const EXPIRY_REMINDER_DAYS = [90, 30, 10, 7, 1];
const MANAGER_REMINDER_DAYS = [10, 7, 1];
const CRON_SECRET = process.env.CRON_SECRET || '';

app.post('/api/cron/check-expiring-domains', async (req, res) => {
  ...
  try {
    ...
    for (const siteDoc of sitesSnap.docs) {
      ...
      const clientDoc = await db.collection('clients').doc(site.clientId).get();
      ...
      const sendAndRecord = async (reminderKey, toAddresses, subject, html) => {
        if (remindersSent.includes(reminderKey)) return;
        try {
          await sendEmail({ to: toAddresses, subject, html });
          await db.collection('sitesWeb').doc(siteDoc.id).update({
            remindersSent: admin.firestore.FieldValue.arrayUnion(reminderKey)
          });
          emailsSent++;
          console.log(`[Cron] Sent reminder "${reminderKey}" for ${domain} → ${toAddresses.join(', ')}`);
        } catch (err) {
          console.error(`[Cron] Failed to send reminder "${reminderKey}" for ${domain}:`, err.message);
        }
      };
...
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6fb;padding:40px 0;">
    <tr><td align="center">
      ...
      for (const days of MANAGER_REMINDER_DAYS) {
        if (daysUntilExp === days && managerEmails.length) {
...
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6fb;padding:40px 0;">
    ...
    console.log(`[Cron] check-expiring-domains: ${processed} sites processed, ${emailsSent} emails sent`);
    res.json({ success: true, processed, emailsSent });
  } catch (err) {
    console.error('[Cron] check-expiring-domains error:', err);
    res.status(500).json({ error: err.message });
  }
});
